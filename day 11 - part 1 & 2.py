...
def trials(data, prev, i = 0, steps = 0, mx_flr_idx = 0):
    if (bests and 0 < steps > min(bests)):
        return
    elif next(iter(x for x in data if x), None) == data[mx_flr_idx]:
        bests.append(steps)
    for idx in [i+1, i-1]:
        if 0 <= idx <= mx_flr_idx:
            ...
            for x in filter(lambda x : len(x) == 1 or check_set(x), dat):
                ...
                if t_new not in prev and (not t_new in prevs or prevs[t_new] <= steps):
                    if not check_current(temp):
                        continue
                    prevs[t_new] = steps+1
                    trials(temp, prev + [t_new], idx, steps+1, mx_flr_idx)      
...
with open("C:\\Advent\\2016\\day11.txt", 'r') as file:
    ...
    trials([x[:] for x in data], [floors], mx_flr_idx = len(data)-1)
    print(min(bests))
